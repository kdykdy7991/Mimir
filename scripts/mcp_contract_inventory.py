...
_CURATED: dict[str, dict[str, Any]] = {
    ...
    "list_documents": {
        "kind": "document_discovery",
        "returns": "A bounded page of authorized document summaries without chunk bodies or host paths.",
        "canonical_params": ["collection", "page", "page_size", "q", "status", "file_type", "folder_id", "tag_ids", "tag_operator", "updated_after", "updated_before", "sort"],
        "compat_aliases": [],
        "pagination": "1-based page pagination; page_size 1..50 (default 20).",
        "limits": "q <= 200 chars; at most 20 tag ids; bounded page size.",
        "errors": [
            {"condition": "unknown or unauthorized collection", "surface": "tool result is_error=true", "message": "collection not found or not accessible"},
            {"condition": "invalid filter or pagination", "surface": "tool result is_error=true", "message": "client InvalidRequestError text"},
            *RATE_BUDGET_ERRORS,
        ],
        "collection_authorization": "Collection access is checked before discovery; only scoped records are returned.",
        "backends": dict(_ALL_BACKENDS),
        "lifecycle": "long_term",
        "expect": {"page_size": {"min": 1, "max": 50, "default": 20}},
    },
    "get_chunk": {
        "kind": "exact_chunk_read",
        "returns": "One full chunk with stable neighbors, source locator, parent id and asset ids.",
        "canonical_params": ["document_id", "chunk_id"],
        "compat_aliases": [{"surface": "input", "alias": "doc_id", "canonical": "document_id", "phase_out": "compatibility window"}],
        "pagination": None,
        "limits": "Both document id and chunk id are required; additionalProperties rejected.",
        "errors": [
            {"condition": "missing ids", "surface": "tool result is_error=true", "message": "'document_id' and 'chunk_id' are required and must be non-empty strings"},
            {"condition": "unknown, unauthorized, or wrong-document chunk", "surface": "tool result is_error=true", "message": "chunk not found or not accessible"},
            *RATE_BUDGET_ERRORS,
        ],
        "collection_authorization": "Document ownership and collection access are resolved before exact chunk ownership is checked.",
        "backends": dict(_ALL_BACKENDS),
        "lifecycle": "long_term",
        "expect": {
            "one_of_required": [["document_id", "chunk_id"], ["doc_id", "chunk_id"]],
            "input_alias_properties": ["doc_id"],
        },
    },
    "search_chunks": {
        "kind": "unified_evidence_search",
        "returns": "Governance-filtered Evidence v1 from one or more authorized collections with deterministic multi-query fusion.",
        "canonical_params": ["query", "alternate_queries", "collection", "collection_ids", "failure_policy", "mode", "filters", "top_k", "rerank", "threshold", "include_content"],
        "compat_aliases": [],
        "pagination": "Bounded top-k response; response budget emits explicit truncation warnings.",
        "limits": "query 1..2000; at most 3 alternate queries; top_k 1..50; at most 20 collections or values per filter dimension.",
        "errors": [
            {"condition": "invalid request or filter", "surface": "tool result is_error=true", "message": "stable validation message"},
            {"condition": "unknown or unauthorized collection", "surface": "tool result is_error=true", "message": "collection not found or not accessible"},
            *RATE_BUDGET_ERRORS,
        ],
        "collection_authorization": "Every requested collection is checked before any retrieval; one denial rejects the whole request. Governance filters only narrow that scope.",
        "backends": dict(_ALL_BACKENDS),
        "lifecycle": "long_term",
        "expect": {
            "required": ["query"],
    ...
    "get_chunk_context": {
        "kind": "bounded_chunk_context",
        "returns": "The authorized hit chunk plus optional active parent and ordered neighbors under one explicit character budget.",
        "canonical_params": ["document_id", "chunk_id", "include", "before", "after", "max_chars"],
        "compat_aliases": [],
        "pagination": None,
        "limits": "before/after 0..10; max_chars 1..100000; truncation is explicit.",
        "errors": [
            {"condition": "invalid bounds", "surface": "tool result is_error=true", "message": "stable validation message"},
            {"condition": "unknown or unauthorized document/chunk", "surface": "tool result is_error=true", "message": "chunk not found or not accessible"},
            *RATE_BUDGET_ERRORS,
        ],
        "collection_authorization": "Document scope and chunk ownership are checked before parent or neighbor reads.",
        "backends": dict(_ALL_BACKENDS),
        "lifecycle": "long_term",
        "expect": {
    ...
    "list_data_sources": {
        "kind": "datasource_discovery",
        "returns": "Secret-free datasource metadata authorized for the current MCP key.",
        "canonical_params": [], "compat_aliases": [], "pagination": None,
        "limits": "Collection whitelist filtering is always applied.",
        "errors": [*RATE_BUDGET_ERRORS],
        "collection_authorization": "Only sources whose collection is authorized are returned.",
        "backends": dict(_ALL_BACKENDS), "lifecycle": "long_term", "expect": {},
    },
    "get_sync_status": {
        "kind": "datasource_status",
        "returns": "Latest secret-free sync state and bounded counters for one datasource.",
        "canonical_params": ["source_id"], "compat_aliases": [], "pagination": None,
        "limits": "One datasource per call.",
        "errors": [
            {"condition": "unknown or unauthorized datasource", "surface": "tool result is_error=true", "message": "data source not found or not accessible"},
            *RATE_BUDGET_ERRORS,
        ],
        "collection_authorization": "Datasource collection access is rechecked before status is returned.",
        "backends": dict(_ALL_BACKENDS), "lifecycle": "long_term",
        "expect": {"required": ["source_id"]},
    },
    "list_sync_failures": {
        "kind": "datasource_failures",
        "returns": "A bounded list of stable sync failure/conflict codes without credentials or remote content.",
        "canonical_params": ["source_id", "limit"], "compat_aliases": [],
        "pagination": "At most 100 most-recent failures.",
        "limits": "limit 1..100, default 20.",
        "errors": [
            {"condition": "unknown or unauthorized datasource", "surface": "tool result is_error=true", "message": "data source not found or not accessible"},
            *RATE_BUDGET_ERRORS,
        ],
        "collection_authorization": "Datasource collection access is rechecked before failures are returned.",
        "backends": dict(_ALL_BACKENDS), "lifecycle": "long_term",
        "expect": {},
    },
}
