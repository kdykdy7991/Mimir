...
> RAG 完整性与独立 Web UI 升级方案见 [`PRODUCTION_WEB_DEV_SPEC.md`](PRODUCTION_WEB_DEV_SPEC.md)。
...
- **LLM 调用层插拔 (LLM Provider Agnostic)**：
    - 核心推理 LLM 通过统一的抽象接口封装，支持**多协议**无缝切换：
        ...
        - **其他云服务**：DeepSeek、MiniMax、Mimo 等第三方 API（均兼容 OpenAI 协议）。
    - **多模态能力**：统一接口支持文本和多模态输入，通过 `capabilities` 属性声明模型能力（如 `{"text"}` 或 `{"text", "vision"}`）；`CapabilityValidator` 在请求进入 Provider 前校验输入类型，不匹配则抛出 `UnsupportedCapabilityError`。
...
设计要点：
- **明确分层职责**：
  - Loader：负责把原始文件解析为统一的 `Document` 对象（`text` + `metadata`；类型定义集中在 `src/core/types.py`）。当前支持 PDF 与 Markdown（`.md` / `.markdown`），经 `LoaderFactory`/`LoaderRegistry`（`src/libs/loader/loader_factory.py`）按文件扩展名分派，单条 `IngestionPipeline` 即可处理混合格式批次。
...
- Loader（统一格式与元数据）
	...
	- **解析与标准化**：
		- PDF：解析为 canonical Markdown，并保留页面、标题和图片引用等定位信息。
		- Markdown：文本主体直接进入 canonical Markdown 流程，保留标题、列表、
		  表格、引用和 fenced code block；metadata 至少包含
		  `source_path`、`doc_type=markdown`、`title/heading_outline`，
		  无分页信息时 `page` 为空。
		- Markdown 默认按 UTF-8（允许 UTF-8 BOM）读取；二进制伪装文件或无法
		  解码的内容必须返回可定位的 Loader 错误，不得写入任何索引。
...
- **统一接口层 (Unified API Abstraction)**：
	...
	- **关键抽象**：
		- `BaseLLM`：统一的 LLM 抽象基类，支持文本和多模态输入。
		- `EmbeddingClient`：暴露 `embed(texts) -> vectors` 方法，统一处理批量请求与维度归一化。

- **多模态能力模型 (Capability Model)**：
	- **设计理念**：现代大模型（GPT-4o、Claude、Qwen-VL 等）是同一个模型同时支持文本、图片、音频等多种输入，而非独立的"文本模型"和"视觉模型"。
	- **能力声明**：通过 `capabilities` 属性声明模型支持的能力集合：
		- 纯文本模型：`{"text"}`
		- 多模态模型：`{"text", "vision"}`
		- 未来扩展：`{"text", "vision", "audio", "video"}`
	- **内容块类型 (ContentBlock)**：采用类型安全的 ContentBlock 设计：
		- `TextBlock`：文本内容
		- `ImageBlock`：图片内容（文件路径或 bytes）
		- `AudioBlock`：音频内容（预留）
		- `VideoBlock`：视频内容（预留）
	- **能力校验 (Capability Validation)**：在请求进入 LLM Provider 前，通过 `CapabilityValidator` 自动校验输入类型是否与模型能力匹配，不匹配则抛出 `UnsupportedCapabilityError`，避免不支持的输入发送到模型。

- **统一消息格式 (Unified Message Format)**：
	- 采用 OpenAI 兼容的消息格式作为统一标准：
		```python
		# 纯文本消息
		...
		```
	- Provider 层负责将统一格式转换为各厂商 API 的原生格式。

- **提供者选项与切换场景**：

| 提供者类型 | 典型场景 | 配置切换点 | 支持能力 |
|---------|---------|-----------|---------|
| **OpenAI** | 通用开发、最新模型尝鲜 | `provider: openai`, `api_key`, `model` | text, vision (GPT-4o) |
| **Azure OpenAI** | 企业合规、私有云部署、区域数据驻留 | `provider: azure`, `endpoint`, `api_key`, `deployment_name` | text, vision (GPT-4o) |
| **DeepSeek** | 成本优化、特定语言优化 | `provider: deepseek`, `api_key`, `model` | text |
| **Ollama / vLLM (本地)** | 完全离线、隐私敏感、无 API 成本 | `provider: ollama`, `base_url`, `model` | text, vision (LLaVA) |

- **技术选型建议**：
	- 本项目采用自研的 `BaseLLM` 抽象基类，配合工厂模式（`llm_factory.py`）实现统一调用接口。已内置 OpenAI Compatible、Azure OpenAI 等 Provider 适配。
...
│   │   ├── llm/                         # LLM 抽象
│   │   │   ├── __init__.py
│   │   │   ├── base_llm.py              # LLM 抽象基类（支持多模态）
│   │   │   ├── content_block.py         # 内容块类型定义（TextBlock/ImageBlock/AudioBlock/VideoBlock）
│   │   │   ├── message.py               # 消息数据结构（Message）
│   │   │   ├── capability_validator.py  # 能力校验器（UnsupportedCapabilityError）
│   │   │   ├── llm_factory.py           # LLM 工厂
│   │   │   └── providers/               # Provider 实现（按协议拆分）
│   │   │       ├── __init__.py
│   │   │       ├── openai_compatible.py # OpenAI Compatible 统一实现（OpenAI/DeepSeek/MiniMax/Mimo/vLLM）
│   │   │       └── azure_openai.py      # Azure OpenAI 实现（支持多模态）
│   │   │
...
| 抽象接口 | 当前默认实现 | 可替换选项 | 能力说明 |
|---------|------------|----------|---------|
| `BaseLLM` | OpenAI Compatible | Azure OpenAI / Ollama / vLLM | 支持文本和多模态，通过 capabilities 属性声明能力 |
| `EmbeddingClient` | OpenAI text-embedding-3 | BGE / Ollama 本地模型 | 文本向量化 |
| `Loader` | PDF Loader（MarkItDown） | Markdown/HTML/Code Loader 等 | 文档解析 |
| `FileIntegrity` | SQLite (`data/db/ingestion_history.db`) | Redis（分布式）/ PostgreSQL（企业级）/ JSON文件（测试） | 文件完整性检查 |
| `Splitter` | RecursiveCharacterTextSplitter | Semantic / FixedLen | 文本切分 |
| `VectorStore` | Chroma | Qdrant / Pinecone / Milvus | 向量存储 |
| `Reranker` | CrossEncoder | LLM Rerank / None (关闭) | 结果重排 |
| `Evaluator` | Ragas | DeepEval / 自定义指标 | 质量评估 |

> **说明**：原 `VisionLLMClient` 已合并到 `BaseLLM`，Vision 不是独立模型类型，而是 LLM 的 capability。调用方通过 `llm.capabilities` 判断模型是否支持视觉输入。
...
│ (Stdio Transport)│
└────────┬────────┘
         │ query + params + images (可选)
         ▼
┌─────────────────┐
│ Capability      │  校验输入类型是否与模型能力匹配
│ Validation      │  TextBlock → text, ImageBlock → vision
│                 │  失败抛出 UnsupportedCapabilityError
└────────┬────────┘
         │ 校验通过
         ▼
┌─────────────────┐
...
# Vision LLM 配置（可选）
# 如果不配置，自动使用主 llm（如果支持 vision 能力）
# 只有当主模型不支持视觉时，才需要单独配置 vision_llm
vision_llm:
  provider: openai          # openai | azure
  model: gpt-4o             # 必须是支持视觉的模型
...
|---------|---------|------|---------|------|
| A1 | 初始化目录树与最小可运行入口 | [x] | 2026-07-09 | 全量目录 + __init__.py + settings.yaml + 3个Prompt模板 + 4个脚本入口 + pyproject.toml |
| A2 | 引入 pytest 并建立测试目录约定 | [x] | 2026-07-09 | pytest markers + smoke tests |
| A3 | 配置加载与校验（Settings） | [x] | 2026-07-09 | Pydantic Settings + YAML + 22 tests |

#### 阶段 B：Libs 可插拔层

| 任务编号 | 任务名称 | 状态 | 完成日期 | 备注 |
|---------|---------|------|---------|------|
| B1 | LLM 抽象接口与工厂（支持多模态） | [x] | 2026-07-09 | BaseLLM + Capability + ContentBlock + LLMFactory + 12 tests |
| B2 | Embedding 抽象接口与工厂 | [x] | 2026-07-09 | BaseEmbedding + EmbeddingFactory + 14 tests |
| B3 | Splitter 抽象接口与工厂 | [x] | 2026-07-09 | BaseSplitter + SplitterFactory + 12 tests |
| B4 | VectorStore 抽象接口与工厂 | [x] | 2026-07-09 | BaseVectorStore + VectorStoreFactory + 17 contract tests |
| B5 | Reranker 抽象接口与工厂（含 None 回退） | [x] | 2026-07-09 | BaseReranker + NoneReranker + RerankerFactory + 13 tests |
| B6 | Evaluator 抽象接口与工厂 | [x] | 2026-07-09 | BaseEvaluator + CustomEvaluator + EvaluatorFactory + 13 tests |
| B7.1 | OpenAI-Compatible LLM 实现（按协议拆分） | [x] | 2026-07-09 | providers/openai_compatible.py + 多模态支持 + 19 smoke tests |
| B7.2 | OpenAI Embedding 实现 | [x] | 2026-07-09 | OpenAIEmbedding + 7 smoke tests |
| B7.3 | Sentence-Transformers Embedding 实现 | [x] | 2026-07-09 | SentenceTransformersEmbedding + HuggingFaceEmbedding + 8 tests |
| B7.4 | Recursive Splitter 默认实现 | [x] | 2026-07-09 | RecursiveSplitter + 7 smoke tests |
| B7.5 | ChromaStore 默认实现 | [x] | 2026-07-09 | ChromaStore + 9 smoke tests |
| B7.6 | LLM Reranker 实现 | [x] | 2026-07-09 | LLMReranker + 9 smoke tests |
| B7.7 | Cross-Encoder Reranker 实现 (BGE/Qwen3) | [x] | 2026-07-09 | CrossEncoderReranker + 9 smoke tests |
| B8 | ~~Vision LLM 抽象接口与工厂集成~~ | [x] | 2026-07-09 | 已合并到 B1，Vision 是 capability 不是独立类型 |

#### 阶段 C：Ingestion Pipeline MVP

| 任务编号 | 任务名称 | 状态 | 完成日期 | 备注 |
|---------|---------|------|---------|------|
| C1 | 定义核心数据类型/契约（Document/Chunk/ChunkRecord） | [x] | 2026-07-22 | 6 个 dataclass (ImageRef/Document/Chunk/ChunkRecord/ProcessedQuery/RetrievalResult) + to_dict/from_dict 序列化 + 14 tests |
| C2 | 文件完整性检查（SHA256） | [x] | 2026-07-22 | FileIntegrityChecker 抽象 + SQLiteIntegrityChecker 默认实现 (WAL 模式) + DEFAULT_DB_PATH=data/db/ingestion_history.db + mark_success/failed 状态机 (failed 可重试) + 并发写入测试 |
| C3 | Loader 抽象基类与 PDF Loader | [x] | 2026-07-22 | BaseLoader (ABC) + PdfLoader (pymupdf) — 文本+位置感知图片提取、图片降级、21 tests |
| C4 | Splitter 集成（调用 Libs） | [x] | 2026-07-22 | DocumentChunker 适配层 — 6 个增值功能 (ID/元数据/index/source_ref/图片按需分发/类型转换) + 24 tests using FakeSplitter |
| C5 | Transform 基类 + ChunkRefiner | [x] | 2026-07-22 | BaseTransform + ChunkRefiner (规则去噪 + 可选 LLM + 降级) + TraceContext 最小实现 + 8 噪声场景 fixture + 40 tests (超过 spec 的 27 目标) |
| C6 | MetadataEnricher | [x] | 2026-07-22 | MetadataEnricher — 规则 (title/summary/tags) + 可选 LLM JSON 输出 + 降级 + 32 tests |
| C7 | ImageCaptioner | [x] | 2026-07-22 | ImageCaptioner — per-image LLM caption + {context}/{image_path} 占位符替换 + 降级标记 has_unprocessed_images + 19 tests |
| C8 | DenseEncoder | [x] | 2026-07-22 | DenseEncoder — 包装 BaseEmbedding → ChunkRecord.dense_vector; 严格顺序、维度校验、错误传播、19 tests |
| C9 | SparseEncoder | [x] | 2026-07-22 | SparseEncoder — BM25 term weights {term: weight}; 两遍算法(语料统计 → per-chunk); ASCII + CJK char n-gram (1+2) tokenization; 58 tests<br>**2026-07-30 演进**：分词从 CJK unigram 升级为 **1+2-gram char n-gram**（每个 CJK 单字 + 每个紧邻双字，仅在 CJK run 内部成 bigram）。原因：unigram 把"考勤"切成"考"+"勤"与大量无关文档共享词项；jieba 引入词典依赖与启动日志噪音；char n-gram 在企业 RAG（紧邻专有名词为主）效果接近 jieba、零依赖。删除 jieba 相关代码、`SparseSettings.tokenizer` 字段、`requirements.txt` 的 jieba 依赖。`from_settings()` 工厂保留作为单一构造入口。 |
| C10 | BatchProcessor | [x] | 2026-07-22 | BatchProcessor — 分批 (5 chunks @ batch_size=2 → 3 batches) + dense+sparse 编码融合 + per-batch 计时到 trace; 18 tests |
| C11 | BM25Indexer（倒排索引+IDF计算） | [x] | 2026-07-22 | BM25Indexer — 倒排索引 + BM25 IDF 公式（`+1` / Lucene 式，与 SparseEncoder 一致）+ build/save/load/query/incremental-add; 35 tests<br>**2026-07-30 修正**：原实现用原始式（无 `+1`），与 SparseEncoder 的 `+1` 式不一致，导致高频词 IDF 为负、命中反而降分。已统一为 `+1` 式并抽出 `_idf()` 单一实现。 |
| C12 | VectorUpserter（幂等upsert） | [x] | 2026-07-22 | VectorUpserter — 确定性 ID (sha256(source_path|chunk_index|content_hash[:8])[:16]) + 批量 upsert + dense_vector 校验 + 26 tests |
| C13 | ImageStorage（图片存储+SQLite索引） | [x] | 2026-07-22 | ImageStorage — SQLite image_index + WAL + 磁盘按 collection 分目录 + 幂等 upsert + 按 collection/doc_hash 批量查询 + 31 tests |
| C14 | Pipeline 编排（MVP 串起来） | [x] | 2026-07-22 | IngestionPipeline — 6 stage 编排 (integrity→load→split→transform→encode→store) + PipelineStageError + 12 integration tests 端到端验证向量+BM25+图片全部产物
| C15 | 脚本入口 ingest.py | [x] | 2026-07-22 | scripts/ingest.py — CLI (--path/--collection/--force/--data-dir/--config) + build_pipeline() 可注入; 14 e2e tests 覆盖 CLI 表面 + 端到端产物 + 二次跳过 + --force 重存 |

#### 阶段 D：Retrieval MVP

| 任务编号 | 任务名称 | 状态 | 完成日期 | 备注 |
|---------|---------|------|---------|------|
| D1 | QueryProcessor（关键词提取 + filters） | [x] | 2026-07-22 | QueryProcessor — query → ProcessedQuery(keywords, filters); 共用 SparseEncoder.tokenizer 保证与索引分词一致; 可选 k:v filter 解析; 27 tests |
| D2 | DenseRetriever（调用 VectorStore.query） | [x] | 2026-07-22 | DenseRetriever — embed query → vector_store.query → RetrievalResult (source="dense"); RetrievalResult 加 chunk_id/text/metadata 便利属性; 24 tests |
| D3 | SparseRetriever（BM25 查询） | [x] | 2026-07-22 | SparseRetriever — keywords → BM25 query → vector_store.get_by_ids 反查 → RetrievalResult (source="sparse") + 索引懒加载缓存; BaseVectorStore 新增 get_by_ids 抽象; 22 tests |
| D4 | RRF Fusion | [x] | 2026-07-22 | reciprocal_rank_fusion — Σ 1/(k+rank) 多路融合; k 可配 (默认 60); deterministic, chunk_id tie-breaker; first-seen-wins 保留 chunk 字段; 22 tests |
| D5 | HybridSearch 编排 | [x] | 2026-07-22 | HybridSearch — D1+D2+D3+D4 编排 + 双路降级 + metadata post-filter + retrieval_multiplier 配置; 19 integration tests |
| D6 | Reranker（Core 层编排 + Fallback） | [x] | 2026-07-22 | RerankerStage — 包装 libs/reranker 后端,异常时返回原列表并打 fallback=True; RerankOutput dataclass; top_m 候选截断; 15 tests |
| D7 | 脚本入口 query.py（查询可用） | [x] | 2026-07-22 | scripts/query.py — CLI (--query/--top-k/--collection/--data-dir/--config/--verbose/--no-rerank) + build_query_components() 可注入; 20 e2e tests; **项目可端到端运行(ingest→query)** |

#### 阶段 E：MCP Server 层与 Tools

| 任务编号 | 任务名称 | 状态 | 完成日期 | 备注 |
|---------|---------|------|---------|------|
| E1 | MCP Server 入口与 Stdio 约束 | [x] | 2026-07-23 | mcp_server/server.py — stdio transport + stderr-only 日志; 7 个 integration tests |
| E2 | Protocol Handler 协议解析与能力协商 | [x] | 2026-07-23 | mcp_server/protocol_handler.py — 复用 mcp 库 Server (替代手写 JSON-RPC); 注册表 + dispatch + 结果归一化; 22 tests |
| E3 | query_knowledge_hub Tool | [x] | 2026-07-23 | mcp_server/tools/query_knowledge_hub.py — HybridSearch + RerankerStage + response_builder; Markdown + structuredContent 引用 |
| E4 | list_collections Tool | [x] | 2026-07-23 | mcp_server/tools/list_collections.py — BM25 索引 + vector store collection 合并; 9 tests |
| E5 | get_document_summary Tool | [x] | 2026-07-23 | mcp_server/tools/get_document_summary.py — 按 doc_id 取元数据 + chunk_count; 5 tests |
| E6 | 多模态返回组装（Text + Image） | [x] | 2026-07-24 | `MultimodalAssembler` — `chunk.image_refs` → `ImageStorage.get` → base64 → `mcp.types.ImageContent`；`MCPResponse.content` 字段；`query_knowledge_hub` 装配 assembler + `ImageNotFoundError` fallback；`as_content_pair()` 新增，`as_pair()` 旧路径保留；21 单元测试覆盖（empty/no-image/with-image/missing/garbage/integration）。**只用 mcp 类型，不引入新抽象**（MCP 库已有 TextContent/ImageContent）；不做 url 模式（YAGNI，无 Web 消费方）；不做 Audio/Video/Pdf/Table 占位。 |

#### 阶段 F：Trace 基础设施与打点

| 任务编号 | 任务名称 | 状态 | 完成日期 | 备注 |
|---------|---------|------|---------|------|
| F1 | TraceContext 增强（finish + 耗时统计 + trace_type） | [x] | 2026-07-23 | TraceContext 增强（trace_type/finish/elapsed_ms/to_dict）+ TraceCollector + 22 单元测试 |
| F2 | 结构化日志 logger（JSON Lines） | [x] | 2026-07-23 | JSONFormatter + get_trace_logger + write_trace + 11 单元测试 |
| F3 | 在 Query 链路打点 | [x] | 2026-07-23 | HybridSearch 6 阶段 + RerankerStage 4 事件 trace 注入（method/elapsed_ms/n_in/n_out/fallback）+ TestTrace 类 8 个集成测试 + reranker_fallback 修复 `time` import + 调整 stale `start` 断言 |
| F4 | 在 Ingestion 链路打点 | [x] | 2026-07-23 | Pipeline 5 阶段 trace 注入（load/split/transform/embed/upsert，method + elapsed_ms）+ 9 集成测试（TestTrace）+ skipped/error/empty_chunks 旁路事件；upsert 阶段覆盖 vector+bm25 双腿 |
| F5 | Pipeline 进度回调 (on_progress) | [x] | 2026-07-23 | on_progress 回调（6 阶段通知：load/split/transform/*/embed/upsert）+ 集成测试覆盖 |

#### 阶段 G：可视化管理平台 Dashboard

| 任务编号 | 任务名称 | 状态 | 完成日期 | 备注 |
|---------|---------|------|---------|------|
| G1 | Dashboard 基础架构与系统总览页 | [x] | 2026-07-24 | `app.py` 多页面导航（st.navigation）+ Overview 渲染 6 个 ComponentCard + ConfigService（从 Settings 读 6 后端配置，含 rerank-none 状态展示）+ start_dashboard.py 真启动 streamlit；4 placeholder page 让导航完整。9 单测覆盖 ConfigService。 |
| G2 | DocumentManager 实现 | [x] | 2026-07-24 | Protocol-typed `DocumentManager(chroma, bm25_indexer, bm25_index, image_storage, integrity)` + 4 存储层扩展（ChromaStore.get_by_metadata / delete_by_metadata / get_collection_stats，BM25Indexer.remove_documents 批量 + 重算 n_docs/avgdl/idf，FileIntegrityChecker.list_processed / get_record_by_path）。19 单测：list / get_detail / delete（含 chroma 失败继续） / stats。 |
| G3 | 数据浏览器页面 | [x] | 2026-07-24 | DataService 只读门面（list + get_detail + get_collection_stats + load_image_b64 / load_image_bytes），data_browser 页三段式：collection 卡片 / 文档表（st.dataframe + selectbox）/ chunk 详情（expander + JSON metadata + 图片预览）。7 单测。 |
| G4 | Ingestion 管理页面 | [x] | 2026-07-24 | IngestionService 包装 scripts.ingest.build_pipeline + IngestionPipeline.run，提供 stage_ingested_file / cleanup_staged_file / ingest_uploaded_file。ingestion_manager 页：st.file_uploader + st.progress（on_progress 透传）+ 删除按钮走 DocumentManager.delete_document。11 单测。 |
| G5 | Ingestion 追踪页面 | [x] | 2026-07-24 | TraceService 读 logs/traces.jsonl → TraceRecord（含 Stage / StageEvent / events），容错跳过损坏行。ingestion_traces 页：历史列表 + selectbox + 阶段耗时柱状图 + event expander。17 单测覆盖 list/get/search/parse/edge cases。 |
| G6 | Query 追踪页面 | [x] | 2026-07-24 | query_traces 页：关键词搜索 + dense vs sparse 并列对比（n_in / n_out）+ rerank fallback 高亮 + 柱状图 + 全 event log。共用 G5 的 TraceService。 |
...
> **进度说明（2026-07-24）**：
> - **A-F 阶段**：全部 100%，与代码 + 测试一致（unit + integration + e2e 共 906 测试全过）。
> - **G 阶段（Dashboard）**：6/6 完成。`src/observability/dashboard/` 从空 `__init__.py` 变成完整 5 页面应用 + 4 个 service。`scripts/start_dashboard.py` 真启动 streamlit。G2 的 4 存储层扩展（ChromaStore / BM25Indexer / FileIntegrityChecker）为 G3-G6 共享。
> - **H 阶段（评估）**：0/5，未做。`src/libs/evaluator/` 的 `BaseEvaluator` + `CustomEvaluator` 是 B6 阶段产物；H1-H5（RagasEvaluator / CompositeEvaluator / EvalRunner / 评估面板 / Recall E2E）未实现。
> - **I 阶段（E2E + 文档）**：2/5，I1（`tests/e2e/test_data_ingestion.py` + `test_query_cli.py`）+ I4（部分契约测试）已落地。I2（Dashboard 冒烟 — G 完成可补）、I3（README 完善）、I5（全链路 E2E 验收 — 需真实数据 + 网络）仍待做。
...
### B1：LLM 抽象接口与工厂（支持多模态）
- **目标**：定义统一的 `BaseLLM` 接口，支持文本和多模态输入；实现 Capability 能力模型和 ContentBlock 类型安全设计；提供 `CapabilityValidator` 在请求进入 Provider 前校验输入类型。
- **修改文件**：
  - `src/libs/llm/base_llm.py` — 扩展接口，添加 capabilities 属性和 validate 方法
  - `src/libs/llm/content_block.py` — 新增：ContentBlock, TextBlock, ImageBlock 类型定义
  - `src/libs/llm/message.py` — 新增：Message 数据结构（可选的类型安全方式）
  - `src/libs/llm/capability_validator.py` — 新增：能力校验器，UnsupportedCapabilityError 异常
  - `src/libs/llm/llm_factory.py` — 简化为单入口，移除 create_vision_llm
  - `tests/unit/test_llm_factory.py`
  - `tests/unit/test_multimodal_llm.py` — 新增：多模态能力测试
- **实现类/函数**：
  - `BaseLLM.capabilities -> set[str]` — 声明模型能力集合（如 {"text"}, {"text", "vision"}）
  - `BaseLLM.chat(messages: list[dict], **kwargs) -> str` — 统一对话接口（支持纯文本和多模态）
  - `BaseLLM.validate(messages: list[dict]) -> None` — 校验消息是否与模型能力匹配
  - `CapabilityValidator.validate(messages, capabilities) -> None` — 能力校验
  - `LLMFactory.create(settings) -> BaseLLM` — 单入口创建 LLM
  - `UnsupportedCapabilityError` — 不支持的能力类型异常
- **能力映射规则**：
  - `TextBlock` / 纯文本 → 需要 `text` 能力
  - `ImageBlock` / `image_url` → 需要 `vision` 能力
  - `AudioBlock` → 需要 `audio` 能力（预留）
  - `VideoBlock` → 需要 `video` 能力（预留）
- **失败行为**：
  - 校验失败时抛出 `UnsupportedCapabilityError`
  - 不调用模型接口，提前拦截
- **验收标准**：
  - FakeLLM 支持文本和多模态输入
  - capabilities 属性正确声明模型能力
  - CapabilityValidator 能正确校验输入类型与模型能力
  - 校验失败时抛出 UnsupportedCapabilityError
  - Factory 单入口创建所有类型的 LLM
- **测试方法**：`pytest -q tests/unit/test_llm_factory.py tests/unit/test_multimodal_llm.py tests/unit/test_capability_validator.py`。
...
### B7.1：OpenAI-Compatible LLM 实现（按协议拆分）
- **目标**：实现统一的 OpenAI Compatible Provider，支持 OpenAI、DeepSeek、MiniMax、Mimo 等所有兼容 OpenAI API 协议的模型；支持多模态消息格式（GPT-4o 等）。
- **修改文件**：
  - `src/libs/llm/providers/__init__.py` — 新增：providers 包
  - `src/libs/llm/providers/openai_compatible.py` — 新增：统一 OpenAI Compatible 实现
  - `src/libs/llm/providers/azure_openai.py` — 新增：Azure OpenAI 实现（支持多模态）
  - `tests/unit/test_llm_providers_smoke.py`（mock HTTP，不走真实网络）
- **删除文件**：
  - `src/libs/llm/openai_llm.py` — 重构为 providers/openai_compatible.py
  - `src/libs/llm/deepseek_llm.py` — 合并到 providers/openai_compatible.py
  - `src/libs/llm/minimax_llm.py` — 合并到 providers/openai_compatible.py
  - `src/libs/llm/mimo_llm.py` — 合并到 providers/openai_compatible.py
  - `src/libs/llm/vllm_llm.py` — 合并到 providers/openai_compatible.py
  - `src/libs/llm/base_vision_llm.py` — 删除（Vision 是 capability）
  - `src/libs/llm/openai_vision_llm.py` — 合并到 providers/openai_compatible.py
- **实现细节**：
  - `OpenAICompatibleLLM` — 统一处理所有 OpenAI 兼容 API
    - 通过 `base_url` 区分不同 provider（OpenAI/DeepSeek/MiniMax/Mimo/vLLM）
    - 支持多模态消息格式（GPT-4o 等）
    - 自动声明 capabilities（根据 model 名称判断是否支持 vision）
  - `AzureOpenAILLM` — Azure 特有差异处理
    - 支持 Azure AD 认证
    - 支持多模态消息格式
- **能力声明**：
  - GPT-4o / GPT-4-turbo → `{"text", "vision"}`
  - GPT-3.5-turbo / DeepSeek / MiniMax / Mimo → `{"text"}`
  - 通过 model 名称自动判断
- **验收标准**：
  - 配置不同 `provider` 时工厂路由正确
  - `chat(messages)` 支持纯文本和多模态消息
  - capabilities 属性正确声明模型能力
  - `validate()` 能正确校验输入类型与模型能力
  - 异常信息清晰（包含 provider 与错误类型）
- **测试方法**：`pytest -q tests/unit/test_llm_providers_smoke.py`。

### B7.2：OpenAI Embedding 实现
- **目标**：补齐 `openai_embedding.py`，支持 OpenAI 官方 API 的 Embedding 调用，支持批量 `embed(texts)`，并可被 mock 测试。
- **修改文件**：
  - `src/libs/embedding/openai_embedding.py`
  - `tests/unit/test_embedding_providers_smoke.py`（mock HTTP）
- **验收标准**：
  - provider=openai 时 `EmbeddingFactory` 可创建，支持 text-embedding-3-small/large 等模型。
  - 空输入、超长输入有明确行为（报错或截断策略由配置决定）。
- **测试方法**：`pytest -q tests/unit/test_embedding_providers_smoke.py`。

### B7.3：Sentence-Transformers Embedding 实现
- **目标**：补齐 `sentence_transformers_embedding.py`，支持本地开源 Embedding 模型推理（如 BGE、E5、GTE 等），支持 GPU 加速，并预留 HuggingFace Transformers 扩展接口。
- **修改文件**：
  - `src/libs/embedding/sentence_transformers_embedding.py`
  - `src/libs/embedding/huggingface_embedding.py`（预留扩展）
  - `tests/unit/test_sentence_transformers_smoke.py`（mock 测试）
- **验收标准**：
  - provider=sentence_transformers 时 `EmbeddingFactory` 可创建。
  - 支持配置 model、device（cpu/cuda）、dimensions。
  - 支持批量 `embed(texts)` 调用。
  - 空输入有明确行为（返回空列表）。
- **测试方法**：`pytest -q tests/unit/test_sentence_transformers_smoke.py`。

### B7.4：Recursive Splitter 默认实现
...
### B7.5：ChromaStore（VectorStore 默认后端）
...
### B7.6：LLM Reranker（读取 rerank prompt）
...
### B7.7：Cross-Encoder Reranker 实现
- **目标**：支持本地 Cross-Encoder 模型进行重排序，无需调用外部 API。
- **支持模型**：
  - `BAAI/bge-reranker-v2-m3`（多语言，经典选择，~568M 参数）
  - `Qwen/Qwen3-Reranker-0.6B`（中文优化，轻量，~0.6B 参数）
  - 其他 Hugging Face Cross-Encoder 模型
- **修改文件**：
  - `src/libs/reranker/cross_encoder_reranker.py`
  - `tests/unit/test_cross_encoder_reranker.py`（mock scorer）
- **实现方式**：
  - 使用 `sentence-transformers` 的 `CrossEncoder` 类加载模型
...
- **验收标准**：
  ...
  - 提供超时/失败回退信号（供 Core 层 fallback 使用）。
- **测试方法**：`pytest -q tests/unit/test_cross_encoder_reranker.py`。

### B8：~~Vision LLM 抽象接口与工厂集成~~（已废弃）
- **状态**：已合并到 B1，删除独立接口
- **原因**：Vision 不是独立模型类型，而是 LLM 的 capability。现代大模型（GPT-4o、Claude、Qwen-VL 等）是同一个模型同时支持文本和图片，而非独立的"文本模型"和"视觉模型"。
- **变更内容**：
  - 删除 `src/libs/llm/base_vision_llm.py`
  - 删除 `src/libs/llm/openai_vision_llm.py`
  - 删除 `LLMFactory.create_vision_llm()` 方法
  - 合并到 `BaseLLM` 统一接口，通过 `capabilities` 属性声明能力
  - 调用方通过 `llm.capabilities` 判断是否支持视觉输入

### B9：~~Azure Vision LLM 实现~~（已废弃）
- **状态**：已合并到 B7.1
- **原因**：Azure OpenAI 是 OpenAI Compatible 的一种，统一实现在 `providers/azure_openai.py` 中
- **变更内容**：
  - 删除 `src/libs/llm/azure_vision_llm.py`
  - 合并到 `providers/azure_openai.py`，支持多模态消息格式
  - 通过 `capabilities` 属性声明是否支持视觉
...
### C3.1：Markdown Loader 与 Loader 路由（新增需求）
- **目标**：实现 `MarkdownLoader`，并由统一 Loader Registry/Factory 按扩展名
  选择解析器，避免 CLI、Web API、Dashboard 分别维护格式判断。
- **修改文件**：
  - `src/libs/loader/markdown_loader.py`
  - `src/libs/loader/loader_factory.py`（或现有等价注册入口）
  - `src/libs/loader/__init__.py`
  - `tests/unit/test_loader_markdown_contract.py`
- **实现要求**：
  - 支持 `.md`、`.markdown`（大小写不敏感），默认 UTF-8/UTF-8 BOM；
  - 保留 Markdown 标题、列表、表格、引用和 fenced code block，不执行内嵌
    HTML/脚本，不主动下载远程图片；
  - 输出统一 `Document`，metadata 至少包含 `source_path`、
    `doc_type=markdown`、`title` 和 `heading_outline`；
  - 文件完整性检查、chunk、embedding、BM25、Chroma 与 Trace 复用现有 Pipeline。
- **验收标准**：
  - Markdown 样例可完成 ingest → query，结构化内容切分后不丢失；
  - 空文件、非法编码、伪装扩展名返回明确错误且不产生残留索引；
  - PDF 既有测试保持通过。
- **测试方法**：`pytest -q tests/unit/test_loader_markdown_contract.py tests/integration/test_ingestion_pipeline.py`。

### C3.2：批量文件摄取编排（新增需求）
- **目标**：在单文件 Pipeline 之上增加批量编排服务；每个文件仍是独立摄取
  单元，拥有独立完整性判断、任务状态、Trace 和错误。
- **建议接口**：
  - `ingest_many(paths, collection, force=False, on_progress=None) -> BatchIngestionResult`
  - `BatchIngestionResult` 包含 `batch_id`、总数及
    `items[{file_name, task_id, status, skipped, error}]`。
- **处理语义**：
  - 同一批文件进入同一 collection，按受控并发度处理；
  - 单文件失败不取消其他文件，不做全批回滚；
  - 重复文件逐个执行现有 SHA256 策略并标记 `skipped`；
  - 进度同时报告 batch 总体和逐文件阶段；重试只处理失败项；
  - 临时文件使用唯一目录，单文件完成后清理，避免同名覆盖和失败残留。
- **验收标准**：
  - 混合上传至少 2 个 PDF/Markdown 文件可全部成功并分别查询；
  - “成功 + 重复 + 非法文件”混合批次返回逐文件结果，成功项不受失败项影响；
  - 同名文件、并发批次和服务重启后的任务查询行为有自动化覆盖。
- **测试方法**：补充应用服务单测、Web API 集成测试和批量 E2E 测试。
...
- **核心功能**：
  - 计算 IDF (Inverse Document Frequency)：`IDF(term) = log((N - df + 0.5) / (df + 0.5) + 1)`
    - 采用 `+1`（Lucene / BM25+）式，**必须与 `SparseEncoder` 入库时的权重公式保持一致**——否则索引期与查询期打分不可比。
    - 原始式（无 `+1`）会让 df > N/2 的词 IDF 为负，命中该词反而拉低文档得分，中文语料尤其容易踩到。
...
- **目标**：在 MCP 响应侧支持图片返回——命中 chunk 含 `image_refs` 时，读取图片、base64 编码后作为 MCP `ImageContent` 块返回。
- **修改文件**：
  - `src/core/response/content_block.py`（新增）：`ContentBlock` 类型别名 + 从 `mcp.types` 重新导出 `TextContent` / `ImageContent`（MCP 库已有这些类型，**不另造抽象**）。
  - `src/core/response/multimodal_assembler.py`（新增）：`MultimodalAssembler` + `ImageNotFoundError`（KeyError 子类）。每 chunk 输出 `TextContent` 后跟 N 个 `ImageContent`（N = `image_refs` 长度）。图片 IO 全部委托 `ImageStorage`。
  - `src/core/response/response_builder.py`（扩展）：`MCPResponse` 增加 `content` 字段 + `as_content_pair()` 方法；`build()` 接受可选 `assembler` 参数。
  - `src/mcp_server/tools/query_knowledge_hub.py`（扩展）：`_build_assembler()` 工厂 + 主流程装配；`ImageNotFoundError` → 自动 fallback 到 text-only 响应（不阻断成功检索）。
  - `src/mcp_server/protocol_handler.py`（小幅扩展）：`_call_tool` 返回类型允许 `ImageContent`。
  - `src/core/response/__init__.py`：暴露新符号。
  - `tests/unit/test_multimodal_assembler.py`（新增 21 测试）。
- **职责边界**：
  - ❌ 不做 OCR / captioning（`ImageCaptioner` 已做）
  - ❌ 不做 embedding（pipeline 已做）
  - ❌ 不做 url 模式（无 Web 消费方，YAGNI）
  - ❌ 不实现 Audio/Video/Pdf/Table block（E6 范围 = 当前可用的 Text + Image）
- **验收标准**：
  - chunk 有 `image_refs` → 输出 `TextContent(text=chunk.text)` + 1 个 `ImageContent` per ref
  - chunk 无 `image_refs` → 仅 `TextContent`
  - `image_id` 在 `ImageStorage` 中不存在 → 抛 `ImageNotFoundError`（不静默）
  - `ImageContent.data` 是合法 base64，可 roundtrip 回原字节
  - `ImageContent.mimeType` 从文件扩展名推断，未知时 fallback `image/png`
  - `MCPResponse.content` 字段非空时是 `list[TextContent | ImageContent]`
  - `MCPResponse.markdown` / `structured` 与是否装配 assembler 无关（向后兼容）
  - `query_knowledge_hub` 在 `ImageNotFoundError` 时降级到 text-only 响应
- **测试方法**：
  ```bash
  pytest -q tests/unit/test_multimodal_assembler.py  # 21/21
  pytest -q tests/integration/test_mcp_server.py     # 7/7 不回归
  pytest -q tests/unit/test_response_builder.py      # 11/11 不回归
  ```
...
- **实现要点**：
  - 文件上传：`st.file_uploader(..., accept_multiple_files=True)` 选择 PDF/Markdown
    文件 + 集合选择；展示批次总体与逐文件状态
  - 摄取触发：调用 `IngestionPipeline.run(on_progress=...)` + `st.progress()` 实时进度
  - 文档删除：在文档列表中提供删除按钮，调用 `DocumentManager.delete_document()`
- **验收标准**：可在 Dashboard 中批量上传 PDF/Markdown 触发摄取、看到总体与
  逐文件进度、单独重试失败项，并删除已有文档。
- **测试方法**：手动验证（批量上传 PDF + Markdown → 观察进度 → 重试失败项 →
  删除 → 确认已移除）。
