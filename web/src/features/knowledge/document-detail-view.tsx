...
import { AlertTriangle, ArrowLeft, Boxes, CheckCircle2, Clock3, FileStack, FileText, ImageIcon, Layers3, MoreHorizontal, Route, Sparkles, Upload } from "lucide-react";
...
function OriginalFilePreview({ document, pdfPage }: { document: DocumentDetail; pdfPage?: number | undefined }) {
  ...
  if (ext === "pdf") return <iframe className="h-[44rem] w-full rounded-lg border border-border bg-white" src={`${url}${pdfPage ? `#page=${pdfPage}` : ""}`} title={`${document.filename} 原文件预览`} />;
