...
import { ArrowUpRight, Boxes, FileStack, Plus, Search, Sparkles, Trash2, X } from "lucide-react";
...
export function CollectionsView() {
  ...
  return <div className="app-container space-y-6">
    ...
    {loading ? <LoadingState label="正在加载知识库" rows={5} /> : error ? <ErrorState {...(apiError ? { code: apiError.code, ...(apiError.requestId ? { requestId: apiError.requestId } : {}) } : {})} {...(error instanceof Error ? { description: error.message } : {})} onRetry={retry} title="无法加载知识库" /> : filtered.length === 0 ? <EmptyState action={!query ? <Button onClick={() => setDialogOpen(true)}><Plus className="size-4" />创建第一个知识库</Button> : undefined} description={query ? "没有匹配当前关键词的知识库。" : "创建知识库后即可上传并组织文档。"} icon={Boxes} title={query ? "没有搜索结果" : "还没有知识库"} /> : <><section aria-label="知识库列表" className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{filtered.map((collection, index) => <article className="glass-surface group relative flex min-h-64 cursor-pointer flex-col rounded-xl p-5 transition-all duration-200 hover:-translate-y-1 hover:border-primary/45 hover:bg-primary/[0.025] hover:shadow-[0_14px_32px_-16px_color-mix(in_srgb,var(--color-primary)_45%,transparent)] focus-within:border-primary/55 focus-within:ring-2 focus-within:ring-primary/20 active:translate-y-0 active:scale-[0.995]" key={collection.id}><Link aria-label={`打开知识库 ${collection.name}`} className="absolute inset-0 rounded-xl focus:outline-none" href={`/collections/${collection.id}`} /><div className="flex items-start justify-between gap-3"><span className={`grid size-11 place-items-center rounded-lg transition-all duration-200 group-hover:scale-105 group-hover:ring-4 group-hover:ring-primary/10 ${index % 3 === 1 ? "bg-accent/10 text-accent" : index % 3 === 2 ? "bg-success/10 text-success" : "bg-primary/10 text-primary"}`}><Boxes className="size-5" /></span><button aria-label={`删除 ${collection.name}`} className="relative z-10 grid size-8 place-items-center rounded-md text-muted-foreground hover:bg-danger/10 hover:text-danger" onClick={() => setDeleteTarget(collection)} type="button"><Trash2 className="size-4" /></button></div><h2 className="mt-5 text-lg font-semibold transition-colors group-hover:text-primary">{collection.name}</h2><p className="mt-2 line-clamp-2 text-sm text-muted-foreground">{collection.description || "暂无描述"}</p><div className="mt-auto flex items-center gap-5 pt-6 text-xs text-muted-foreground"><span className="inline-flex items-center gap-1.5"><FileStack className="size-3.5" />{collection.document_count ?? 0} 文档</span><span className="inline-flex items-center gap-1.5"><Sparkles className="size-3.5" />{(collection.chunk_count ?? 0).toLocaleString()} 片段</span></div><div className="mt-4 flex items-center justify-between border-t border-border pt-4 transition-colors group-hover:border-primary/20"><span className="text-[0.6875rem] text-muted-foreground">更新于 {formatDate(collection.updated_at)}</span><ArrowUpRight aria-hidden="true" className="size-4 -translate-x-1 translate-y-1 text-muted-foreground opacity-45 transition-all duration-200 group-hover:translate-x-0 group-hover:translate-y-0 group-hover:text-primary group-hover:opacity-100" /></div></article>)}</section>{(nextCursor === undefined ? data?.page_info.next_cursor : nextCursor) ? <div className="flex justify-center"><Button loading={loadingMore} onClick={loadMore} variant="secondary">加载更多</Button></div> : null}</>}
